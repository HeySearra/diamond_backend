...
class ReadAuth(models.Model):
    ...
    def get_user_list(self):
        return [e.user for e in ReadMem.objects.filter(read_auth=self)]
...
class CommentAuth(models.Model):
    ...
    def get_user_list(self):
        return [e.user for e in CommentMem.objects.filter(comment_auth=self)]
...
class WriteAuth(models.Model):
    ...
    def get_user_list(self):
        return [e.user for e in WriteMem.objects.filter(write_auth=self)]


class ShareAuth(models.Model):

    user = models.ManyToManyField(to='user.User', related_name='shared_doc', null=True, blank=True)
    owner = models.ManyToManyField(to='user.User', related_name='shareable_doc', null=True, blank=True)
